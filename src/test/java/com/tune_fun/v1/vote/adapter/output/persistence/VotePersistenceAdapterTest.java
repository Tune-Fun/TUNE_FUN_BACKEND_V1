...
class VotePersistenceAdapterTest {
    ...
    /**
     * Method under test:
     * {@link VotePersistenceAdapter#scrollUserLikedVotePaper(String, Long, LocalDateTime, Integer)}
     */
    @Test
    @DisplayName("유저가 좋아요 누른 투표 게시물 스크롤 조회")
    void testScrollUserLikedVotePaper() {
        // given
        String username = "username";
        Long lastId = 2L;
        LocalDateTime lastTime = LocalDateTime.of(2024, 10, 10, 0, 0);
        LocalDateTime today = LocalDateTime.of(2024, 10, 7, 0, 0);
        int count = 5;
        long likeCount = 100;
        Long votePaperId = 1L;

        UserInteractedVotePaper userInteractedVotePaper = new UserInteractedVotePaper(
                votePaperId,
                ...
                "authorUserName",
                "authorNickname",
                VotePaperOption.ALLOW_ADD_CHOICES,
                today.minusDays(7),
                today.plusDays(7),
                today.plusDays(8),
                true,
        ...
        ScrollableVotePaper mockScrollableVotePaper = new ScrollableVotePaper(
                userInteractedVotePaper.id(),
                userInteractedVotePaper.uuid(),
                userInteractedVotePaper.title(),
                userInteractedVotePaper.authorUsername(),
                null,
                userInteractedVotePaper.authorNickname(),
                today.until(userInteractedVotePaper.voteEndAt(), ChronoUnit.DAYS),
                "0",
        ...
        given(votePaperRepository.findLikedByUsernameBeforeLastTimeAndLastId(username, lastId, lastTime, count))
                .willReturn(List.of(userInteractedVotePaper));
        given(votePaperStatisticsRepository.findLikeCountMap(Set.of(votePaperId)))
                .willReturn(Map.of(votePaperId, likeCount));
        given(votePaperRepository.findParticipatedVotePaperIdsByUsername(username, Set.of(votePaperId)))
                .willReturn(List.of(votePaperId));
        given(votePaperMapper.scrollableUserInteractedVotePaper(userInteractedVotePaper, likeCount, true, true))
                .willReturn(mockScrollableVotePaper);

        // when
        Window<ScrollableVotePaper> result = votePersistenceAdapter.scrollUserLikedVotePaper(username, lastId, lastTime, count);

        // then
        then(votePaperRepository).should(times(1)).findLikedByUsernameBeforeLastTimeAndLastId(username, lastId, lastTime, count);
        then(votePaperStatisticsRepository).should(times(1)).findLikeCountMap(Set.of(votePaperId));
        then(votePaperRepository).should(times(1)).findParticipatedVotePaperIdsByUsername(username, Set.of(votePaperId));
        then(votePaperMapper).should(times(1)).scrollableUserInteractedVotePaper(userInteractedVotePaper, likeCount, true, true);
        ...
        assertThat(scrollableVotePaper.id()).isEqualTo(userInteractedVotePaper.id());
        assertThat(scrollableVotePaper.uuid()).isEqualTo(userInteractedVotePaper.uuid());
        assertThat(scrollableVotePaper.title()).isEqualTo(userInteractedVotePaper.title());
        assertThat(scrollableVotePaper.authorUsername()).isEqualTo(userInteractedVotePaper.authorUsername());
        assertThat(scrollableVotePaper.authorProfileImageUrl()).isNull();
        assertThat(scrollableVotePaper.authorNickname()).isEqualTo(userInteractedVotePaper.authorNickname());
        assertThat(scrollableVotePaper.remainDays()).isEqualTo(7L);
        ...
        assertThat(scrollableVotePaper.userLiked()).isTrue();
        assertThat(scrollableVotePaper.userVoted()).isTrue();
    }

    /**
     * Method under test:
     * {@link VotePersistenceAdapter#scrollUserVotedVotePaper(String, Long, LocalDateTime, Integer)}
     */
    @Test
    @DisplayName("유저가 투표한 투표 게시물 스크롤 조회")
    void testScrollUserVotedVotePaper() {
        // given
        String username = "username";
        Long lastId = 2L;
        LocalDateTime lastTime = LocalDateTime.of(2024, 10, 10, 0, 0);
        ...
        given(votePaperRepository.findParticipatedByUsernameBeforeLastId(username, lastId, lastTime, count))
                .willReturn(List.of(userInteractedVotePaper));
        given(votePaperStatisticsRepository.findLikeCountMap(Set.of(votePaperId)))
                .willReturn(Map.of(votePaperId, likeCount));
        given(votePaperRepository.findLikedIdsByUsernameAndIds(username, Set.of(votePaperId)))
                .willReturn(List.of(votePaperId));
        given(votePaperMapper.scrollableUserInteractedVotePaper(userInteractedVotePaper, likeCount, true, true))
                .willReturn(mockScrollableVotePaper);

        // when
        Window<ScrollableVotePaper> result = votePersistenceAdapter.scrollUserVotedVotePaper(username, lastId, lastTime, count);

        // then
        then(votePaperRepository).should(times(1)).findParticipatedByUsernameBeforeLastId(username, lastId, lastTime, count);
        then(votePaperStatisticsRepository).should(times(1)).findLikeCountMap(Set.of(votePaperId));
        then(votePaperRepository).should(times(1)).findLikedIdsByUsernameAndIds(username, Set.of(votePaperId));
        then(votePaperMapper).should(times(1)).scrollableUserInteractedVotePaper(userInteractedVotePaper, likeCount, true, true);
        List<ScrollableVotePaper> scrollableVotePapers = result.getContent();
        assertThat(scrollableVotePapers).hasSize(1);
        ScrollableVotePaper scrollableVotePaper = scrollableVotePapers.getFirst();
